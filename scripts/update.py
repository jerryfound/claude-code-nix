#!/usr/bin/env python3
"""Refresh hashes.json with the latest stable Claude Code release from npm.

Reads the registry metadata of the @anthropic-ai/claude-code meta package
to find the latest stable version, then records the tarball URL and
sha512 integrity hash of every supported platform package. No tarballs
are downloaded and no Nix installation is required.
...
META_PACKAGE = "@anthropic-ai/claude-code"
...
def platform_entry(target: str, version: str) -> dict:
    meta = fetch_json(f"{REGISTRY}/@anthropic-ai/claude-code-{target}/{version}")
    return {
